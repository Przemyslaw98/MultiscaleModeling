...
Imagebox::Imagebox(int s,int cX,int cY,int preset,int valueA,int valueB,int border,int neighborhood,int valueC,const char* label):Fl_Box(0,0,cX*s,cY*s,label){
  srand(time(NULL));
...
int Imagebox::handle(int event){
  ...
  if(event!=FL_PUSH||!window_choose->shown())
    return 0;
  if(current_image[Fl::event_x()/scale][Fl::event_y()/scale]==0)
    current_image[Fl::event_x()/scale][Fl::event_y()/scale]=1;
  else current_image[Fl::event_x()/scale][Fl::event_y()/scale]=0;
  draw_image();
