...
Fl_Value_Input *input_MC=(Fl_Value_Input *)0;

Fl_Value_Input *input_kt=(Fl_Value_Input *)0;
...
Fl_Round_Button *btn_grains=(Fl_Round_Button *)0;

static void cb_btn_grains(Fl_Round_Button*, void*) {
  callback_grains();
}

Fl_Round_Button *btn_gr=(Fl_Round_Button *)0;

static void cb_btn_gr(Fl_Round_Button*, void*) {
  callback_gr();
}


Fl_Round_Button *btn_energy=(Fl_Round_Button *)0;

static void cb_btn_energy(Fl_Round_Button*, void*) {
  callback_energy();
}

Fl_Round_Button *btn_en=(Fl_Round_Button *)0;

static void cb_btn_en(Fl_Round_Button*, void*) {
  callback_en();
}
...
int main() {
  { window_main = new Fl_Double_Window(800, 320, "Rozrost ziaren");
    ...
    { input_MC = new Fl_Value_Input(600, 200, 32, 32, "Monte Carlo iterations:");
      input_MC->minimum(1);
      input_MC->maximum(100);
      input_MC->value(10);
    }
    { input_kt = new Fl_Value_Input(600, 240, 32, 32, "kt:");
      input_kt->minimum(0.1);
      input_kt->maximum(6);
      input_kt->value(1);
    }
    { new Fl_Box(680, 200, 25, 25, "Default view:");
    }
    { btn_gr = new Fl_Round_Button(650, 230, 96, 25, "Grains");
      btn_gr->down_box(FL_ROUND_DOWN_BOX);
      btn_gr->callback((Fl_Callback*)cb_btn_gr);
      btn_gr->set();
    }
    { btn_en = new Fl_Round_Button(650, 260, 96, 25, "Energy");
      btn_en->down_box(FL_ROUND_DOWN_BOX);
      btn_en->callback((Fl_Callback*)cb_btn_en);
    }
...
void callback_grains(){
  btn_grains->value(1);
  btn_energy->value(0);
  result->draw_image();
  window_result->redraw();
}
void callback_energy(){
  btn_grains->value(0);
  btn_energy->value(1);
  result->draw_energy();
  window_result->redraw();
...
int callback_start() {
  ...
  { window_result = new Fl_Double_Window(x*s, y*s+32, "Results");
    ...
    { btn_grains = new Fl_Round_Button(0, 0, 96, 25, "Grains");
      btn_grains->down_box(FL_ROUND_DOWN_BOX);
      btn_grains->callback((Fl_Callback*)cb_btn_grains);
      if(default_view==false)
        btn_grains->set();
      btn_grains->deactivate();
    }
    { btn_energy = new Fl_Round_Button(80, 0, 96, 25, "Energy");
      btn_energy->down_box(FL_ROUND_DOWN_BOX);
      btn_energy->callback((Fl_Callback*)cb_btn_energy);
      if(default_view==true)
        btn_energy->set();
      btn_energy->deactivate();
    }
...
void callback_grow(void*){
  if(result->grow()>0)
    ...
    result->make_energy_array();
    if(input_MC->value()>0){
      kt=input_kt->value();
      if(kt<0.1)kt=0.1;
      if(kt>6)kt=6;
      MC_iterations=input_MC->value();
      Fl::add_timeout(result->speed,callback_monte_carlo);
    }
    else{
    btn_grains->activate();
    btn_energy->activate();
...
void callback_monte_carlo(void*){
  if(result->monte_carlo(MC_iterations,kt,default_view)>0)
    Fl::add_timeout(result->speed,callback_monte_carlo);
  else{
    btn_grains->activate();
    btn_energy->activate();
